...
bool CN3FXPartBillBoard::Load(HANDLE hFile)
{
	...
	if (m_iVersion>=6) ReadFile(hFile, &m_bRotationRate, sizeof(bool), &dwRWC, nullptr); // implement m_bRotationRate?
	if (m_iVersion>=7) ReadFile(hFile, &m_bOnScreen, sizeof(bool), &dwRWC, nullptr); // implement m_bOnScreen?
	if (m_iVersion>=8)
	{
		char skipBuffer[13];
		ReadFile(hFile, skipBuffer, sizeof(skipBuffer), &dwRWC, nullptr);
	}
	if (m_iVersion>=9)
	{
		char skipBuffer12[12];
		ReadFile(hFile, skipBuffer12, sizeof(skipBuffer12), &dwRWC, nullptr);
	}
